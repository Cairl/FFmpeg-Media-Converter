...
output_ext = input('Enter the target file extension: ').lstrip('.')
...
files_to_convert = [os.path.join(source_location, f) for f in os.listdir(source_location)] if os.path.isdir(source_location) else [source_location]
converted_files, sr_files = [], []
...
for file_path in converted_files:
    os.rename(file_path, os.path.join(os.path.dirname(file_path), os.path.basename(file_path).replace('[FF] ', '')))
